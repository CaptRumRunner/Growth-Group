...
Admin PIN unlock happens in a popup. Admin (small button, top right,
same line as the title) edits address, time, upcoming dates, and the
category list (name from a typical-categories dropdown, description,
how many families/items are needed) -- plus a "clear for new night"
button to wipe sign-ups clean between gatherings.
...
CATEGORY_ICONS = {
    "Main Dish": ICON_PLATE,
    "Side Dish": ICON_BOWL,
    "Salad": ICON_BOWL,
    "Dessert": ICON_CUPCAKE,
    "Drinks": ICON_CUP,
    "Snacks": ICON_BOX,
    "Bread": ICON_BREAD,
    "Appetizer": ICON_UTENSILS,
    "Plates / Utensils": ICON_UTENSILS,
    "Other": ICON_BOX,
}
...
def initials(name):
    """Two-letter badge from a family surname (e.g. 'Griffith' -> 'GR')."""
    letters = name.strip()
...
st.markdown(
    ...
    .dates-card {background:#fff; border:1px solid #e7ddce; border-radius:12px; padding:0.6rem 0.85rem; margin-bottom:0.6rem; font-size:0.85rem;}
    .dates-card .label {font-size:0.68rem; color:#8a8171; font-weight:700; text-transform:uppercase; letter-spacing:0.03em;}
    ...
    .cat-avatar {
        ...
        display:flex; align-items:center; justify-content:center; background:#5b3a29;
    }
    ...
    .signee-row {
        ...
        background:#f4f9f5; border-radius:8px; padding:0.4rem 0.65rem; margin-top:0.4rem; font-size:0.87rem;
    }
    .signee-avatar {
        width:26px; height:26px; border-radius:50%; display:inline-flex; align-items:center; justify-content:center;
        color:white; font-size:0.68rem; font-weight:800; margin-right:0.5rem; flex-shrink:0;
    }
    .signee-name {font-weight:700; color:#1f2937;}
    .signee-detail {color:#5c6b60;}
...
if len(event["upcoming_dates"]) > 1:
    more = event["upcoming_dates"][1:]
    st.markdown(
        f"""<div class="dates-card"><span class="label">Also Coming Up</span><br>{'<br>'.join(more)}</div>""",
        unsafe_allow_html=True,
    )
...
with h1:
    st.markdown(f"<div class='section-title'>{ICON_UTENSILS.replace('white','#1f4d3d')} Food Sign Up</div>", unsafe_allow_html=True)
with h2:
    ...
    if signed_families:
        for fam, d in signed_families.items():
            ...
            st.markdown(
                ...
                <span class="signee-detail">({d['count']} people -- {d['category']})</span>
                </div>""",
                unsafe_allow_html=True,
            )
    else:
...
for cat in event["categories"]:
    ...
    with cc2:
        ...
        if st.button(btn_label, key=btn_key, use_container_width=True):
            sign_up_dialog(name, signed_families)
        if remaining == 0:
            st.markdown(
...
st.markdown("<div class='version-tag'>v5.0</div>", unsafe_allow_html=True)

# ----------------------------------------------------------------------
# Admin panel
# ----------------------------------------------------------------------
if st.session_state.admin_open and st.session_state.admin_ok:
    st.divider()
    st.markdown("**Admin**")
    ...
    with st.form("categories_form"):
        st.markdown("**Food categories** (pick from typical types, set how many are needed)")
        df = pd.DataFrame(event["categories"])
        edited = st.data_editor(
            df, num_rows="dynamic", hide_index=True, use_container_width=True,
            column_config={
                "name": st.column_config.SelectboxColumn("Category", options=TYPICAL_CATEGORIES, required=True),
                "desc": st.column_config.TextColumn("Description"),
                "slots": st.column_config.NumberColumn("Qty Needed", min_value=0, step=1),
            },
            key="cat_editor",
        )
    ...
    st.caption(
        ...
        "Categories and quantities stay -- adjust them above, then update the dates."
    )
